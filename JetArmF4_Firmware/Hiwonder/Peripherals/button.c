...
void button_task_handler(ButtonObjectTypeDef *self, uint32_t period)
{
    ...
    if(self->last_pin_filtered == self->last_pin_raw && !self->last_pin_raw) { /* 按钮状态没有改变且为松开状态，即状态机状态不会发送转移，直接返回 */
		if(self->ticks_count > self->combin_th && self->combin_counter != 0) {  /* 若之前是多连击，则清除连击计数 */
