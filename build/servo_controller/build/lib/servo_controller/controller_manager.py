...
class ControllerManager(Node):
    def __init__(self, name):
        rclpy.init()
        super().__init__(name, allow_undeclared_parameters=True, automatically_declare_parameters_from_overrides=True)  # Allow undeclared parameters. 允许未声明的参数
        self.joints = ['joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'r_joint']

        # Read configuration parameters. 读取配置参数
        self.base_frame = self.get_parameter('base_frame').value
        
        # Initialize the trajectory_controller. trajectory_controller的初始化
        self.controllers = {}
        connected_ids = {}
        ...
        # Instantiate the servo motor management node. 实例化舵机管理节点
        self.servo_manager = ServoManager(connected_ids)
        # self.servo_manager.connect()  # Check if the specified servo motors are connected. 检查是否有给定的舵机已连接
        ...
        # Ensure that ros_robot_controller has been fully initialized. 确保ros_robot_controller已完成初始化
