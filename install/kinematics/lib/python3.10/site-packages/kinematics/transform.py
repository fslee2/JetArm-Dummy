...
# Link length in meters 连杆长度(m)
# The height of the base, here the origin of the first coordinate system coincides with that of the second coordinate system. 底座的高度，这里把第一个坐标系和第二个坐标的原点重合到一起了
...
# When calculating tool_link, the value is set to link3 + tool_link because the origin of the end-effector coordinate system is aligned with that of the previous link. 计算tool_link时取值为link3 + tool_link，因为把末端的坐标系原点和前一个重合到一起了
# The tool_link refers to the actual length of the gripper. 这里的tool_link指实际上的夹持器长度
link3 = 0.05445583202
tool_link = 0.112

# The joint angle limits depend on potential collisions and the servo's rotation range. 各关节角度限制，取决于是否碰撞以及舵机的转动范围
# An extra margin of 0.2 is added to account for numerical instability during calculations, which may cause the computed values to slightly exceed the set limits. 多加0.2为了防止计算时数值的不稳定，会比设定值大一点点
joint1 = [-120.2, 120.2]
joint2 = [-180.2, 0.2]
joint3 = [-120.2, 120.2]
joint4 = [-200.2, 20.2]
...
#  Servo pulse width range, midpoint value, and the corresponding angle range and midpoint value.  舵机脉宽范围，中位值，对应的角度范围，中位值
...
# Determine whether a matrix is a rotation matrix. 判断是否为旋转矩阵
