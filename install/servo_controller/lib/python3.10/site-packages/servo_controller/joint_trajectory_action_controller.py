...
class JointTrajectoryActionController:
    ...
    def follow_trajectory_callback(self, goal_handle):
        ...
        if num_points == 0:  # Return immediately if there are no trajectory points. 如果没有轨迹点则立刻返回
        ...
        if not traj.points[0].positions:  # If empty. 如果为空
        ...
        # If traj.header.stamp is zero, use the current time. 如果 traj.header.stamp 为零，则使用当前时间
        ...
        for seg in range(len(trajectory)):
            ...
            # Print feedback message here to indicate whether feedback was received. 在这里打印反馈消息表示程序是否接收到了反馈
