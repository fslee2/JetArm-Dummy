...
class ActionGroupController:
    ...
    def run_action(self, action_name):
        '''
        When executing an action group, the stop signal cannot be sent during execution. 运行动作组，无法发送stop停止信号
        :param action_name: Name of the action group, as a string. 动作组名字 ， 字符串类型
        :param times:  Number of repetitions.  运行次数
        :return:
        '''
        ...
        if os.path.exists(action_name) is True:
            if self.running_action is False:
                ...
                while True:
                    ...
                    else:   # Exits only after execution is complete. 运行完才退出
                        break
