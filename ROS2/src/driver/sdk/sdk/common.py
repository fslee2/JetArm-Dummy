...
def warp_affine(image, points, scale=1.0):
    """
     Simple alignment. Calculate the angle of the line connecting the two points. Rotate the picture around the image center to make the line horizontal can be used to align the face. 简单的对齐，计算两个点的连线的角度，以图片中心为原点旋转图片，使线水平可以用在人脸对齐上
...
def perspective_transform(img, src, dst, debug=False):
    """
    Perform perspective transformation: converting a skewed view of a road image taken from an oblique angle into a top-down view, aligning the camera's viewpoint parallel to the road.
...
def world_to_pixels(world_points, K, T):
    ...
    Args:
        world_points: A list of world coordinates (x, y, z). 世界坐标点列表
        K: Camera intrinsic matrix. 相机内参矩阵
        T: Camera extrinsic matrix [R|t]. 外参矩阵 [R|t]
    Returns:
        pixel_points: A list of pixel coordinates. 像素坐标点列表
    """
    pixel_points = []
