...
def main(args=None):
    ...
    try:
        while rclpy.ok():
            controller.set_servo_position(0.5, ((4, 200),))  # Set the servo ID 4 to position 200. 设置舵机 ID 4 到位置 200
            time.sleep(0.5)  # Wait for 0.5 seconds. 等待 0.5 秒
            controller.set_servo_position(0.5, ((4, 500),))  # Set the servo ID 4 to position 500. 设置舵机 ID 4 到位置 500
            time.sleep(0.5)  # Wait for 0.5 seconds. 等待 0.5 秒
    except KeyboardInterrupt:
        pass
    finally:
        controller.destroy_node()  # Clean the node 清理节点
        rclpy.shutdown()  # Turn off ROS 2. 关闭 ROS 2
