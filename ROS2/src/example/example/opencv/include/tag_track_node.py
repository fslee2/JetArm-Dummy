...
class TagTrackNode(Node):
    ...
    def main(self):
        while self.running:
            ...
            tags = sorted(tags, key=lambda tag: tag.tag_id) # If the result is already sorted in ascending order, manual sorting is not necessary. 出来就是升序排列的不需要手动进行排列
            common.draw_tags(result_image, tags, corners_color=(0, 0, 255), center_color=(0, 255, 0))# Draw the recognized labels on the image. 将识别到的标签在图像上画出来
