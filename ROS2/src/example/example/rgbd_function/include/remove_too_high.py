...
# Maintain equal height. 等高保持
# Find the tallest object in the recognition area. 找出识别区域高度最高的物体
# If the object height exceeds the threshold. 如果物体高度超过阈值
# Remove the tallest object. 则将最高的物体移除
# The program is tested using a 30x30x30mm wooden block and a 40x40x40mm wooden block. 程序测试使用的是 30x30x30mm的木块和 40x40x40mm的木块
        ...
        while self.running:
            ...
            try:
                ...
                for obj in contours:
                    ...
                    pose_end = np.matmul(self.hand2cam_tf_matrix, common.xyz_euler_to_mat(position, (0, 0, 0)))  # Relative coordinates of the converted end. 转换的末端相对坐标
                    world_pose = np.matmul(self.endpoint, pose_end)  # Convert to the robotic arm's world coordinates. 转换到机械臂世界坐标
