    ...
    def main(self):
        ...
        while self.running:
            ...
            if len(boxes) > 0:
                ...
                self.detected_face = min(self.detected_face, 20) # Ensure that the count is never greater than 20. 让计数总是不大于20
                ...
                if self.detected_face >= 5:
                    center = [box_center(box) for box in boxes] # Calculate the center coordinate of all human faces. 计算所有人脸的中心坐标
                    dist = [distance(c, (o_w / 2, o_h / 2)) for c in center] # Calculate the distance from the center of each detected face to the center of the screen. 计算所有人脸中心坐标到画面中心的距离
                    face = min(zip(boxes, center, dist), key=lambda k: k[2]) # Identify the face with the minimum distance to the center of the screen. 找出到画面中心距离最小的人脸
                ...
                result_image = show_faces( result_image, bgr_image, boxes, keypoints) # Display the detected faces and facial key points on the screen. 在画面中显示识别到的人脸和脸部关键点
