...
class Grasp(Node):
    ...
    def grasp_callback(self, grasp):
        self.set_target(grasp)  # Call set target method 调用设置目标方法
        if grasp.mode == 'pick':
    ...
    def execute_pick_sequence(self):
        # The sequence of processing grasping movements 处理夹取的动作序列
        queue_list = [
            [self.move_toward, 1],
    ...
    def execute_place_sequence(self):
        # The sequence of processing placing movements 处理放置的动作序列
        queue_list = [
            [self.move_toward, 1],
            [self.move_approach, 1],
            [self.move_target, 1],
