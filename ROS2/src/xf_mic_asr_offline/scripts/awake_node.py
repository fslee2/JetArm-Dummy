...
class CircleMic:
    ...
    def switch_mic(self, mic="mic6_circle"):
        # mic: Microphone array type, mic4: Linear 4-microphone, mic6: Linear 6-microphone, mic6_circle: Circular 6-microphone. mic：麦克风阵列类型，mic4：线性4麦，mic6：线性6麦， mic6_circle：环形6麦
        param ={
            "type": "switch_mic",
            "content": {
                "mic": "mic6_circle"
    ...
    # Obtain version information. 获取版本信息
    ...
    # Wake word Replacement, Light customization. 唤醒词更换（浅定制）
    ...
    # Data serial port transmission 数据串口发送
    def send_data(self, header, args):
        ...
        self.serialHandle.write(packet)  # Send the master control message. 发送主控消息

    # Send data 发送数据
    def send(self, header, args):
        self.serialHandle.write([0xa5, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, 0xa5, 0x00, 0x00, 0x00, 0xb0])  # Send a handshake request 发送握手请求
        while True:
            result = None
            ...
            if recv_data == header_[0]:
                ...
                if recv_data == header_[1]:
                    ...
                    else:  # No confirmation received. 没有收到确认
                        recv_data = self.serialHandle.read(4)
                        self.serialHandle.read((recv_data[1] << 8 | recv_data[0]) + 1)

                        time.sleep(0.1)
                        self.serialHandle.write(
                            [0xa5, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, 0xa5, 0x00, 0x00, 0x00, 0xb0])  # Continue to send the handshake request. 继续发送发送握手请求
    ...
    # Detect whether it is awakened and the corresponding Angle of awakening. 检测是否唤醒以及唤醒对应角度
                            ...
                            if m is not None:
                                ...
                                if m is not None:
                                    ...
                                    if self.flag_pub is not None and self.angle_pub is not None:
                                        ...
                                        angle = self.val_map(angle, 0, 360, 360, 0) + 240  # Compatible with circles. 和圆形兼容
                                        if angle >= 360:
                                            angle -= 360
                                        msg = Int32()
                                        msg.data = int(angle)
...
class AwakeNode(Node):
    ...
    # Obtain the version information of the microphone. 获取麦克风版本信息
    def get_setting_srv(self, request, response):
        res = self.mic.get_setting()
        response.message = str(res)
        return response

    # Set up the wake word service. 设置唤醒词服务
