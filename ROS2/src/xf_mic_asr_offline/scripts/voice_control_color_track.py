    ...
    def words_callback(self, msg):
        ...
        if words is not None and words not in ['唤醒成功(wake-up-success)', '休眠(Sleep)', '失败5次(Fail-5-times)',
            ...
            if words == '追踪红色' or words == 'track red object':
                ...
                req.data = 1.0 # The tracking colors are 0.0 for no tracking, 1.0 for red, 2.0 for green, and 3.0 for blue. 追踪的颜色 0.0为不追踪， 1.0为红色， 2.0为绿色，3.0为蓝色
                ...
                req = SetFloat64.Request()
                req.data = 2.0 # The tracking colors are 0.0 for no tracking, 1.0 for red, 2.0 for green, and 3.0 for blue. 追踪的颜色 0.0为不追踪， 1.0为红色， 2.0为绿色，3.0为蓝色
                res = self.send_request(self.set_color_client, req)
                if res.success:
                ...
                req = SetFloat64.Request()
                req.data = 3.0 # The tracking colors are 0.0 for no tracking, 1.0 for red, 2.0 for green, and 3.0 for blue. 追踪的颜色 0.0为不追踪， 1.0为红色， 2.0为绿色，3.0为蓝色
                res = self.send_request(self.set_color_client, req)
                if res.success:
